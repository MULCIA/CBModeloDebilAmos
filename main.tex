...
La demostraci'on, por inducci'on d'ebil sobre $j$, es: sea $\sigma \in T^{1}$. Existe $x \in \{1,...,n\}$ tal que
...
Sea $j < n - 1 (j \geq 1)$ y supongamos que el resultado es cierto para $j$. Sea $\sigma \in T^{j+1}$. Sea
$x \in \{1,...,n\}$ tal que $\sigma \in \bar{T}_{x}^{j+1} = quitar()$.

Entonces, se verifica que $\sigma ...$.

De la hipotesis de inducci'on se tiene que $|A_{\sigma,j}| = ...$.
